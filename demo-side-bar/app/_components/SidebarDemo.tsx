...
export function SidebarDemo({
  ...
  const links = [
    {
      ...
      href: "/",
      icon: (
        <HiOutlineHome className=" text-white bg-clip-text dark:text-neutral-200 h-5 w-5 flex-shrink-0" />
      ),
    },
  ...
  return (
    <div
      className={cn(
        "rounded-lg flex flex-col md:flex-row bg-gray-100 dark:bg-neutral-800 flex-1 max-w-full mx-auto border border-neutral-200 dark:border-neutral-700 overflow-hidden",
            ...
            <div className="mt-8 flex flex-col gap-4 ">
              {links.map((link, idx) => (
                <SidebarLink
                  key={idx}
                  link={link}
                  className="text-white hover:text-secondary-light text-lg"
                />
              ))}
            </div>
          </div>
...
export const Logo = () => {
  return (
    <Link
      href="#"
      className=" flex space-x-2 items-center text-sm text-zinc-50 py-1 relative z-20"
    >
      ...
      <motion.span
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className=" text-white dark:text-white whitespace-pre"
      >
        Acet Labs
      </motion.span>
    </Link>
...
export const LogoIcon = () => {
  return (
    <Link
      href="#"
      className="flex space-x-2 items-center seco-sm text-white py-1 relative z-20"
    >
